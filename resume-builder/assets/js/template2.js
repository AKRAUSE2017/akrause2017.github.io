...
firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
        var currUser = user;
        if (document.referrer == 'https://lamp.eng.fau.edu/~akrause2017/resume-application/www/adjustResume2.html' || document.referrer == 'https://lamp.eng.fau.edu/~akrause2017/resume-application/www/profile.html') {
...
function setPDF(){
    var doc = new jsPDF();
    var divider = "______________________________________________________________________________________________"
    var bullet = "•     "
    var spacing = 20;
    ...
    if(dbBusiness1 != ""){
        ...
        doc.setFont("courier");
        spacing = spacing + 10;
        doc.text("PROFESSIONAL EXPERIENCE", 20, spacing);
        ...
        var splitWork1 = doc.splitTextToSize(dbJobDescription1, 150);
        doc.text(splitWork1, 25, spacing);
        ...
        if(dbBusiness2 != ""){
            ...
            spacing = spacing + 5;
            doc.text(bullet, 20, spacing);
            var splitWork2 = doc.splitTextToSize(dbJobDescription2, 150);
            doc.text(splitWork2, 25, spacing);
        }
        ...
        if(dbBusiness3 != ""){
            ...
            spacing = spacing + 5;
            doc.text(bullet, 20, spacing);
            var splitWork3 = doc.splitTextToSize(dbJobDescription3, 150);
            doc.text(splitWork3, 25, spacing);
        }
    ...
    if(dbInstitution1 != ""){
        ...
        if(dbInstitution2 != ""){
            ...
            doc.setFont("times");
            spacing = spacing + 5;
            doc.text(dbDegree2 + " " + dbEnroll1 + " - " + dbGrad1, 20, spacing);
            spacing = spacing + 5;
            doc.text(bullet, 20, spacing);
    ...
    doc.setFont("courier");
    spacing = spacing + 10;
    doc.text("ADDITIONAL SKILLS", 20, spacing);
    ...
    spacing = spacing + 10;
    doc.text(bullet, 20, spacing);
    doc.text(dbSkills1, 25, spacing);
    spacing = spacing + 7;
    doc.text(bullet, 20, spacing);
    doc.text(dbSkills2, 25, spacing);
    spacing = spacing + 7;
    doc.text(bullet, 20, spacing);
    doc.text(dbSkills3, 25, spacing);
...
function adjust(){
    window.location.href = "adjustResume2.html";
}
